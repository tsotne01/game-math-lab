...
function Slider({ label, value, onChange, min = 0, max = 1, step = 0.01, id }: SliderProps) {
  const sliderId = id || `slider-${label.toLowerCase().replace(/\s+/g, '-')}`;
  return (
    <div className="mb-3">
      <div className="flex justify-between text-sm mb-1">
        <label htmlFor={sliderId} className="text-text-secondary">{label}</label>
        <span className="text-accent font-mono" aria-live="polite">{value.toFixed(2)}</span>
      </div>
      <input
        id={sliderId}
        type="range"
        ...
        aria-valuemax={max}
        aria-valuenow={value}
        className="w-full h-2 bg-bg-secondary rounded-lg appearance-none cursor-pointer accent-accent touch-manipulation"
      />
    </div>
  );
}
...
function TextureSlot({ label, value, onChange, icon }: TextureSlotProps) {
  ...
  const slotId = `texture-${label.toLowerCase().replace(/\s+/g, '-')}`;
  ...
  return (
    <div className="bg-bg-secondary rounded-lg p-2 text-center">
      <div className="flex items-center justify-center gap-1 text-xs text-text-secondary mb-1">
        {icon}
        <span id={`${slotId}-label`}>{label}</span>
      </div>
      <div 
        role="button"
        tabIndex={0}
        aria-labelledby={`${slotId}-label`}
        aria-describedby={`${slotId}-desc`}
        className={`w-full h-12 rounded border-2 border-dashed cursor-pointer transition-all touch-manipulation
      ...
      >
        {!value && (
          <div className="h-full flex items-center justify-center">
            <svg className="w-5 h-5 text-text-secondary" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" aria-hidden="true">
      ...
      <span id={`${slotId}-desc`} className="sr-only">
        {value ? `${label} texture loaded. Click to change.` : `Click to upload ${label} texture`}
      </span>
      <input
        ref={inputRef}
        type="file"
      ...
      {value && (
        <button
          ...
          className="text-xs text-red-400 hover:text-red-300 mt-1 focus:outline-none focus:underline"
    ...
    <div className="bg-bg-card border border-border rounded-xl overflow-hidden">
      <div className="grid grid-cols-1 lg:grid-cols-3">
        ...
        <div className="p-4 bg-bg-secondary/50 overflow-y-auto max-h-[500px]">
          {/* Object Type */}
          <div className="mb-4">
            <label id="object-shape-label" className="text-sm text-text-secondary block mb-2">Object Shape</label>
            <div className="grid grid-cols-4 gap-1" role="radiogroup" aria-labelledby="object-shape-label">
              {(['sphere', 'cube', 'torus', 'cylinder'] as ObjectType[]).map(type => (
                <button
                  ...
                  className={`px-2 py-1 rounded text-xs capitalize transition-all touch-manipulation
          ...
          {/* Environment */}
          <div className="mb-4">
            <label htmlFor="environment-select" className="text-sm text-text-secondary block mb-2">Environment</label>
            <select
              id="environment-select"
              value={environment}
              onChange={(e) => setEnvironment(e.target.value as EnvironmentPreset)}
              aria-label="Select environment lighting"
              className="w-full bg-bg-secondary border border-border rounded px-3 py-2 text-sm touch-manipulation"
            >
          ...
          {/* Material Presets */}
          <div className="mb-4">
            <label id="presets-label" className="text-sm text-text-secondary block mb-2">Quick Presets</label>
            <div className="flex flex-wrap gap-1" role="group" aria-labelledby="presets-label">
              {(['gold', 'chrome', 'plastic', 'rubber', 'glass'] as const).map(preset => (
                <button
                  key={preset}
                  onClick={() => loadPreset(preset)}
                  aria-label={`Apply ${preset} material preset`}
                  className="px-2 py-1 bg-bg-secondary hover:bg-bg-secondary/70 rounded text-xs capitalize touch-manipulation focus:outline-none focus:ring-2 focus:ring-accent"
                >
                  {preset}
                </button>
              ))}
          ...
          {/* Base Color */}
          <div className="mb-4">
            <label htmlFor="base-color" className="text-sm text-text-secondary block mb-2">Base Color</label>
            <div className="flex gap-2">
              <input
                ...
                className="w-12 h-8 rounded cursor-pointer border-0 touch-manipulation"
              />
              <input
                type="text"
                value={settings.color}
          ...
          {/* Emissive */}
          <div className="mb-4">
            <label htmlFor="emissive-color" className="text-sm text-text-secondary block mb-2">Emissive</label>
            <div className="flex gap-2 mb-2">
              <input
                ...
                aria-label="Select emissive glow color"
                className="w-12 h-8 rounded cursor-pointer border-0 touch-manipulation"
              />
          ...
          <div className="mb-4">
            <label className="flex items-center gap-2 cursor-pointer">
              <input
                ...
                className="w-4 h-4 accent-accent touch-manipulation"
              />
          ...
          <button
            ...
            aria-label="Export material settings as JSON file"
            className="w-full bg-accent text-black font-semibold py-2 rounded hover:bg-accent/90 transition-colors flex items-center justify-center gap-2 touch-manipulation focus:outline-none focus:ring-2 focus:ring-white focus:ring-offset-2 focus:ring-offset-bg-secondary"
          >
...
export function UVMappingVisualizer() {
  ...
  const handleTouchStart = (e: React.TouchEvent<HTMLCanvasElement>) => {
    ...
    const rect = e.currentTarget.getBoundingClientRect();
    const x = ((touch.clientX - rect.left) / rect.width) * 256;
    const y = ((touch.clientY - rect.top) / rect.height) * 256;
    paint(x, y);
  };
      ...
      <div className="flex items-center justify-between mb-4">
        ...
        <div className="flex gap-2" role="tablist" aria-label="View mode">
          <button
            ...
            aria-controls="uv-3d-panel"
            className={`px-3 py-1 rounded text-sm touch-manipulation focus:outline-none focus:ring-2 focus:ring-accent ${view === '3d' ? 'bg-accent text-black' : 'bg-bg-secondary'}`}
          >
            3D View
          </button>
          <button
            ...
            aria-controls="uv-2d-panel"
            className={`px-3 py-1 rounded text-sm touch-manipulation focus:outline-none focus:ring-2 focus:ring-accent ${view === '2d' ? 'bg-accent text-black' : 'bg-bg-secondary'}`}
          >
            2D UV
          </button>
        </div>
        ...
        <div className="space-y-3">
          <div className="flex gap-2 items-center">
            <input
              ...
              aria-label="Select paint brush color"
              className="w-10 h-10 rounded cursor-pointer border-0 touch-manipulation"
            />
            <div className="flex-1">
              <label htmlFor="brush-size-slider" className="text-xs text-text-secondary">Brush Size: {brushSize}px</label>
              <input
                ...
                className="w-full accent-accent touch-manipulation"
              />
            </div>
            <button
              onClick={clearCanvas}
              aria-label="Clear canvas and reset to checker pattern"
              className="px-3 py-1 bg-red-500/20 text-red-400 rounded text-sm hover:bg-red-500/30 touch-manipulation focus:outline-none focus:ring-2 focus:ring-red-400"
            >
              Clear
            </button>
          </div>
          <canvas
            ...
            width={256}
            height={256}
            className="w-full aspect-square rounded-lg cursor-crosshair border border-border touch-none"
            aria-label="UV painting canvas. Draw on this canvas to paint on the 3D sphere."
            role="img"
    ...
    <div className="bg-bg-card border border-border rounded-xl p-4">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div className="space-y-4">
          <div>
            <label id="pattern-label" className="text-sm text-text-secondary block mb-2">Pattern</label>
            <div className="flex gap-2" role="radiogroup" aria-labelledby="pattern-label">
              <button
                onClick={() => setPattern('brick')}
                role="radio"
                aria-checked={pattern === 'brick'}
                className={`px-3 py-1 rounded text-sm touch-manipulation focus:outline-none focus:ring-2 focus:ring-accent ${pattern === 'brick' ? 'bg-accent text-black' : 'bg-bg-secondary'}`}
              >
                Brick
              </button>
              <button
                ...
                role="radio"
                aria-checked={pattern === 'noise'}
                className={`px-3 py-1 rounded text-sm touch-manipulation focus:outline-none focus:ring-2 focus:ring-accent ${pattern === 'noise' ? 'bg-accent text-black' : 'bg-bg-secondary'}`}
              >
                Noise
              </button>
            </div>
          ...
          <div>
            <label className="flex items-center gap-2 cursor-pointer mb-3">
              <input
                id="enable-normal-map"
                type="checkbox"
                checked={showNormal}
                onChange={(e) => setShowNormal(e.target.checked)}
                aria-label="Toggle normal map effect"
                className="w-4 h-4 accent-accent touch-manipulation"
              />
  ...
  return (
    <div className="bg-bg-card border border-border rounded-xl p-4">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div className="space-y-4">
          <div>
            <label htmlFor="pbr-base-color" className="text-sm text-text-secondary block mb-2">Base Color</label>
            <input
              id="pbr-base-color"
              type="color"
              value={color}
              onChange={(e) => setColor(e.target.value)}
              aria-label="Select base material color"
              className="w-full h-10 rounded cursor-pointer border-0 touch-manipulation"
            />
          </div>
          ...
          <div>
            <label htmlFor="pbr-environment" className="text-sm text-text-secondary block mb-2">Environment</label>
            <select
              id="pbr-environment"
              value={environment}
              onChange={(e) => setEnvironment(e.target.value as EnvironmentPreset)}
              aria-label="Select environment lighting preset"
              className="w-full bg-bg-secondary border border-border rounded px-3 py-2 text-sm touch-manipulation"
            >
              {ENVIRONMENT_PRESETS.map(preset => (
                <option key={preset} value={preset}>{preset.charAt(0).toUpperCase() + preset.slice(1)}</option>
...
export function MetalnessRoughnessMatrix() {
  ...
  return (
    <div className="bg-bg-card border border-border rounded-xl p-4">
      ...
      <div className="mb-4">
        <label htmlFor="matrix-base-color" className="text-sm text-text-secondary block mb-2">Base Color</label>
        <input
          id="matrix-base-color"
          type="color"
          value={baseColor}
          onChange={(e) => setBaseColor(e.target.value)}
          aria-label="Select base color for all spheres"
          className="w-24 h-8 rounded cursor-pointer border-0 touch-manipulation"
        />
      </div>
  ...
  return (
    <div className="bg-bg-card border border-border rounded-xl p-4">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        ...
        <div className="space-y-4">
          <div>
            <label htmlFor="comparison-color" className="text-sm text-text-secondary block mb-2">Color</label>
            <input
              id="comparison-color"
              type="color"
              value={color}
              onChange={(e) => setColor(e.target.value)}
              aria-label="Select material color for comparison"
              className="w-full h-8 rounded cursor-pointer border-0 touch-manipulation"
            />
          </div>
